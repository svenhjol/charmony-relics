...
public class ShieldRelic implements RelicDefinition {
    @Override
    public String id() {
        return "shield";
    }
    ...
    public MutableComponent name(RandomSource random) {
        return Relics.feature().handlers.prefixedRelicName(Component.translatable("item.minecraft.shield"), random);
    }
    ...
    public List<ResourceKey<Enchantment>> validEnchantments(HolderGetter.Provider provider) {
        return TagHelper.getResourceKeys(provider.lookupOrThrow(Registries.ENCHANTMENT), Tags.ON_SHIELD);
    }
}
