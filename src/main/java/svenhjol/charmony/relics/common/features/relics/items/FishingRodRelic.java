...
public class FishingRodRelic implements RelicDefinition {
    @Override
    public String id() {
        return "fishing_rod";
    }
    ...
    public MutableComponent name(RandomSource random) {
        return Relics.feature().handlers.prefixedRelicName(Component.translatable("item.minecraft.fishing_rod"), random);
    }
    ...
    public List<ResourceKey<Enchantment>> validEnchantments(HolderGetter.Provider provider) {
        return TagHelper.getResourceKeys(provider.lookupOrThrow(Registries.ENCHANTMENT), Tags.ON_FISHING_ROD);
    }
}
